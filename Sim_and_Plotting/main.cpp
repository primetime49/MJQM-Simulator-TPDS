...
int main (int argc, char *argv[]){
    ...
    int sampling_method = sampling_input[argv[4]]; //0->exp, 1->pareto, 2->det, 3->unif, 4->Bounded_pareto
    ...
    std::string classes_filename = path + input_filename + ".txt";
    read_classes(classes_filename, p, sizes, mus);

    std::string lambdas_filename = path + "arrRate_" + input_filename + ".txt"; 
    read_lambdas(lambdas_filename, arr_rate);

    std::ofstream outputFile("Results/sinthethicExp/" + input_filename + "-nClasses" + std::to_string(sizes.size()) + "-" + sampling_name[sampling_method] + ".csv", std::ios::app);
